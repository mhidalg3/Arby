...
**Escalation tiers.** The hot path never improvises on money. When it can't proceed — session expiry, 2FA re-challenge, an unexpected/novel response — it escalates through a pluggable `RecoveryHandler` interface:

1. **Cold path (recovery).** Resolve the *out-of-band, non-time-critical* condition (re-auth, clear a 2FA/captcha, refresh a token), then hand control back to the deterministic hot path. **Today: human-in-the-loop over Telegram** (alert → operator acts → resume). Because `RecoveryHandler` is an interface, this can later be swapped for a **fully agentic implementation (openclaw)** that performs the recovery autonomously — *recovery only, never the time-critical placement loop*. Latency is acceptable here (by definition not time-critical), which is exactly why an agent is safe in the cold path and not in the hot path. The hot path stays deterministic regardless of which `RecoveryHandler` is installed.
2. **Frozen path (last resort).** If recovery fails — the agent can't resolve it, or recovery itself keeps failing — execution **freezes**: halt all placement, persist state, fire a high-priority alert, and require explicit **human takeover** to resume. Fail-stopped, never fail-open.

So the evolution is: *deterministic hot path* (fixed) → *cold-path recovery* (human now, openclaw later) → *frozen-path human takeover* (always the final backstop). Telegram carries both the routine notifications (arb found, placing, placed/fill, naked exposure, kill-switch tripped) and the escalation/freeze alerts.
